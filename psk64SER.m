...
% 64 QAM Modulation Signal Error Rate 
...
for k=1:iteration
    ...
    subplot(1,2,1);
    hold on;
    plot(real(x(56)),imag(x(56)),"b.","MarkerSize",7);
    hold off;
    ...
    compansated_angle = atand(imag(x(56))/real(x(56)));
    if (compansated_angle < 2.8) || (compansated_angle > 8.4)
        error1 = error1 + 1;
    end
    
    compansated_angle = atand(imag(y(56))/real(y(56)));
    if (compansated_angle < 2.8) || (compansated_angle > 8.4)
        error2 = error2 + 1;
    end
    ...
    subplot(1,2,2);
    hold on;
    plot(real(y(56)),imag(y(56)),"b.","MarkerSize",7);
    hold off;
    
end

l1 = t*cosd(2.8) + 1i.*t.*sind(2.8);
l2 = t*cosd(8.4) + 1i.*t.*sind(8.4);
...
for k = 1:2
    ...
    for j = 1:4  
        for ii = 0:15
            if ii == 0
                plot(real(x_int(900*(j-1)+1)),imag(x_int(900*(j-1)+1)),"k+","MarkerSize",10,"LineWidth",1);
            else
                plot(real(x_int(900*(j-1)+(ii*56))),imag(x_int(900*(j-1)+(ii*56))),"k+","MarkerSize",10,"LineWidth",1);
            end
